...
`frontend-feature` 스킬은 사용자 흐름, 화면 상태, 컴포넌트 경계, API 계약, 접근성, 반응형 검증을 어떻게 생각해야 하는지 설명한다.

`frontend-verification` 스킬은 구현된 화면을 실제 브라우저에서 상태별로 확인하고, 콘솔 오류, 반응형 깨짐, 접근성 문제, 백엔드 계약 누락을 검증하는 방법을 설명한다.
